...
function RecordOnService() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const carResponse = await axios.get('http://192.168.8.116:8000/api/carslist/');
                setCars(carResponse.data);

                const serviceResponse = await axios.get('http://192.168.8.116:8000/api/servicelist/');
    ...
    const handleSubmit = async () => {
        ...
        if (selectedHours < 9 || (selectedHours === 19 && selectedMinutes > 0) || selectedHours > 19) {
        ...
        try {
            ...
            const response = await axios.post('http://192.168.8.116:8000/api/appointmentlist/', {
            ...
            console.error('Ошибка добавления записи:', error.response?.data || error.message);
        }
    };
    ...
    useEffect(() => {
        const fetchTimes = async () => {
            ...
            try {
                ...
                const response = await axios.get('http://192.168.8.116:8000/api/records/time/', {
    ...
    return (
        <View style={styles.container}>
            ...
            <ModalSelector
                data={services.map(service => ({ key: service.id, label: service.title }))}
                initValue={selectedService ? services.find(service => service.id === selectedService)?.title : 'Выберите услугу'}
                onChange={(option) => setSelectedService(option.key)}
                style={styles.input}
            >
                <Text style={styles.inputText}>
                    {selectedService ? services.find(service => service.id === selectedService)?.title : 'Выберите услугу'}
                </Text>
            </ModalSelector>
